...
// This is some data we pass to the callback. In this example, we pass a JSON
// query so our JSON parser can parse the record, and a count to update the
...
double bench_rapidjson_engine(char *data, long length, json_query_t query,
                              int queryno) {
    ...
    printf(
        "RapidJSON:\t\t\x1b[1;33mResult: %ld (Execution Time: %f "
        "seconds)\x1b[0m\n",
        matching, elapsed);
    return elapsed;
}

double bench_sparser_engine(char *data, long length, json_query_t jquery,
                            ascii_rawfilters_t *predicates, int queryno) {
    bench_timer_t s = time_start();
...
static int getLine(const char *prmpt, char *buff, size_t sz) {
    ...
    if (fgets(buff, sz, stdin) == NULL) return END_OF_FILE;
...
void print_queries(int num_queries) {
    for (int i = 0; i < num_queries; i++) {
        printf(
            "\x1b[1;34mQuery %d:\x1b[0m -------------\x1b[1;31m%s\x1b[0m\n "
            "---------------------\n",

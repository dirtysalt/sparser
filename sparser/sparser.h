 ...
 *
 * NOTE: This version of sparser is simplified and relies on the two-way search
 * algorithm in glibc's memmem/strstr implementation rather than on explicit
 * x86-based vector instructions. Glibc's memmem is highly optimized for small
 * strings, so Sparser's optimizer still selects an effective filter cascade by
 * measuring passthrough rates.
 *
 * This version of the schedule currently only considers conjunctive queries,
 * but extending it to disjunctions is relatively straightforward and will be
...
//
// The `input` pointer passed via the callback is guarnateed to be in the data
// buffer passed to `sparser_search` and `sparser_calibrate`. The callback
// should not modify the length of the data buffer, as this can cause errors
 ...
 *
 */
void search_schedules(ascii_rawfilters_t *predicates, search_data_t *sd,
                      int len, int start, int *result, const int result_len) {
#ifdef DEBUG
    ...
    if (len == 0) {
        ...
        for (int i = 0; i < result_len; i++) {
            for (int j = 0; j < result_len; j++) {
                // Skip records with the same "source" string (e.g., don't
                // consider both 'Athe' and 'hena' from Athena)
                if (i != j && predicates->sources[result[i]] ==
                                  predicates->sources[result[j]]) {
                    SPARSER_DBG(
                        "\x1b[0;33mskipped\x1b[0m due to duplicate source!\n");
                    long end = rdtsc();
                    sd->skipped++;
...
Cycles/Parse: %lu\n\
Total Time: %f\n",
        t->sampling_total, t->searching_total, t->grepping_total,
        t->cycles_per_schedule_avg,
        ((double)t->skipped) / ((double)(t->processed + t->skipped)) * 100.0,
        t->cycles_per_parse_avg, t->total);
}
...
sparser_stats_t *sparser_search(char *input, long length, BYTE delimiter,
    ...
    while (current_record_start < input_last_byte) {
        ...
        // TODO for binary data, we don't want to do this: Instead, we search
        // forward until we find a match, and then scan forward in the callback
        // until the match point.
        current_record_end =
            (char *)memchr(current_record_start, delimiter,
        ...
        for (unsigned i = 0; i < query->count; i++) {
            if (memmem(current_record_start, record_length, query->queries[i],
                       query->lens[i]) == NULL) {
                break;
            }
        ...
        // Update to point to the next record. The top of the loop will update
...
Fraction False Positives: %f",
             stats->records, stats->total_matches, stats->sparser_passed,
